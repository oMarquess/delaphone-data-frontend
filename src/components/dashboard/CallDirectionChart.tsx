...
// Color palette for direction types
...
export default function CallDirectionChart({ data, isLoading = false }: CallDirectionChartProps) {
  // Process data safely
  const formattedData = useMemo(() => {
    try {
      ...
      return data.map(item => ({
        name: item.direction ? (LABELS[item.direction as keyof typeof LABELS] || item.direction) : 'Unknown',
        value: item.count || 0,
        percentage: item.percentage || 0,
      }));
    } catch (error) {
      console.error('Error formatting direction chart data:', error);
      return [];
    }
  }, [data]);
  ...
  if (formattedData.length === 0) {
    return (
      <div className="h-64 flex items-center justify-center bg-gray-50 dark:bg-gray-900 rounded">
        <p className="text-gray-500 dark:text-gray-400">No direction data available for the selected time period</p>
      </div>
    );
  }
